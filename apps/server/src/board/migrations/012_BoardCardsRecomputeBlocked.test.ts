...
describe("migration 012: recompute blocked", () => {
  it.effect("clears a flag left by an archived dependency and keeps every honest one", () =>
    Effect.gen(function* () {
      ...
      // Genuinely blocked — unfinished, live dependency at Ready.
      yield* insertCard({
        id: "really-blocked",
        stage: "ready",
        dependsOn: ["dep-open"],
      ...
      // Blocked flag set below Ready, where the gate does not apply at all.
      ...
      // A dependency id whose card is gone still counts as unmet.
      yield* insertCard({ id: "dangling", stage: "ready", dependsOn: ["nothing"], blocked: 0 });
