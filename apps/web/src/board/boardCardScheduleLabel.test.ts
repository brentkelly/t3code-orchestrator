...
describe("boardCardScheduleLabel", () => {
  it("is absent on an unscheduled card, so it costs the common card nothing", () => {
    expect(
      boardCardScheduleLabel({ scheduledStartAt: null, done: false, nowMs: NOW_MS }),
    ).toBeNull();
  });
  ...
  it("says nothing on a done card, which is not going to move on a timer", () => {
    expect(
      boardCardScheduleLabel({ scheduledStartAt: at(21), done: true, nowMs: NOW_MS }),
    ).toBeNull();
  });

  it("renders no pill for a value it cannot read, rather than an empty chip", () => {
    expect(
      boardCardScheduleLabel({ scheduledStartAt: "soon", done: false, nowMs: NOW_MS }),
    ).toBeNull();
  });
});
