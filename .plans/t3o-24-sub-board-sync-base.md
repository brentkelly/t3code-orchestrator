...
slice guarantees only that a child _starting_ later cuts from a base containing merged siblings
(the post-merge local fast-forward); a child already _running_ keeps its original base. Two
...
1. A stale child's PR into the integration branch can conflict, surfacing today as a merge-time
   ...
   at the worst moment.
2. Worse and quieter: a child whose review rounds converged on a diff against the _old_ base can
...
- Any change for top-level cards: their base (the default branch) moves too, but that is the
  ...
  siblings are _created_ to collide; top-level cards are not.
...
_Why an agent and not server-side `git rebase`:_ a conflicted rebase needs judgment; a mechanical
...
_Why intercept the crossing rather than enqueue on entry to merge:_ the merge-role stage's
...
`baseTipAtRoundStart` too (its base is the default branch; the _head_ moving is what the
