...
merge, and each conflict has to be *understood* rather than re-applied.
...
<!-- Populated by t3o-02 onward. Keep sorted by file path.
     Shape: "one-line append", "delegating call", "block" — anything that is a block
     is a candidate for moving into a T3o-owned module. -->
...
before the MVP is proved. The value we want from early merges is *information* — how bad are the
