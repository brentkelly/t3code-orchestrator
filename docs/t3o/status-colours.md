...
- **One notice per card, and it is ranked.** The card face has room for a single status word, and it
  ...
  then `No PR`, then `Publishing` (a live publish-on-done run — blue, running work on a Done card),
  then `Publish needs you` (a failed publish-on-done attempt on a Done card — amber,
